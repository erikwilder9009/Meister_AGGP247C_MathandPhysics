...
public class DrawereringTool : MonoBehaviour
{
    ...
    public static void SCEscape(SCCanvas.Scene scene)
    {
        Color DrawC = Color.red;
        Vector3 A = new Vector3(scene.frameB.x - 50 * scene.Zoom, scene.frameB.y - 10 * scene.Zoom);
        Vector3 B = new Vector3(scene.frameB.x - 10 * scene.Zoom, scene.frameB.y - 10 * scene.Zoom);
        Vector3 C = new Vector3(scene.frameB.x - 10 * scene.Zoom, scene.frameB.y - 50 * scene.Zoom);
        Vector3 D = new Vector3(scene.frameB.x - 50 * scene.Zoom, scene.frameB.y - 50 * scene.Zoom);
        if (MathTool.PointInRectangle(Input.mousePosition, A, B, C, D))
        {
